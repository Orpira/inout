...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ isset($registros_horarios) ? route('registro_horario.update', $registros_horarios->id) : route('registro_horario.store') }}" method="POST">
                ...
                <fieldset class="border p-3 rounded">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <input type="datetime-local" name="entrada" id="entrada" class="form-control" {{ $registros_horarios->estado === 'FINALIZADO' || $registros_horarios->estado === 'cerrado automaticamente' ? 'disabled' : '' }} value="{{ $registros_horarios->entrada ?? '' }}" required>
                        </div>
                        <div class="col-md-6">
                            <label for="salida">Salida</label>
                            <input type="datetime-local" name="salida" id="salida" class="form-control" {{ $registros_horarios->estado === 'FINALIZADO' || $registros_horarios->estado === 'cerrado automaticamente' ? 'disabled' : '' }} value="{{ $registros_horarios->salida ?? '' }}" required>
                        </div>
                    </div>
                </fieldset>
            </div>
            <div class="form-group">
                <fieldset class="border p-3 rounded">
                    <legend class="w-auto px-2">Extras Calculadas</legend>

                    <div class="row">
                        <div class="col-md-6">
                            <label for="ordinaria">Diurnas</label>
                            <input type="text" name="ordinaria" id="ordinaria" class="form-control" value="{{ $registros_horarios->extrasordinarias ?? '' }}" disabled>
                        </div>
                        <div class="col-md-6">
                            <label for="ordinariaNocturna">Nocturnas</label>
